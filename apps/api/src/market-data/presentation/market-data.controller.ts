...
export class MarketDataController {
  ...
  async getIndicators(
    ...
    const snapshot = await this.cache.getIndicator(
      provider as ExchangeProvider,
      symbol.toUpperCase(),
      interval,
    );
  ...
  async getProviderStatus(@Param('provider') provider: string) {
    const status = await this.cache.getStreamStatus(provider as ExchangeProvider);
  ...
  @Post('backfill')
  @ApiOperation({ summary: 'Trigger historical backfill (Requires Auth in production)' })
