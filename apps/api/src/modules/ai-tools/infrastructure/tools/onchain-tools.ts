...
> {
  ...
  readonly description = 'Fetch verified daily active-address, transaction-count and adjusted transfer-volume metrics from Coin Metrics';
  ...
  readonly cachePolicy = { type: 'SOURCE_TIMESTAMP_AWARE' as const, ttlSeconds: 900 };
  readonly retryPolicy = { maxAttempts: 2, baseDelayMs: 300, maxDelayMs: 1500, retryableErrors: ['TIMEOUT'] };
  ...
  async execute(
    ...
    const rows = Array.isArray(payload.data)
      ...
      : [];
    if (rows.length === 0) {
      throw new Error(`Coin Metrics has no verified on-chain coverage for ${asset.toUpperCase()}`);
    }
