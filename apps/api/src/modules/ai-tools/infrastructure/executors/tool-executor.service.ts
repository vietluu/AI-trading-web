...
export class ToolExecutorService {
  ...
  public async execute(toolName: string, rawArgs: unknown, context: ToolExecutionContext): Promise<ToolResult> {
    const startedAt = new Date();
    const tool = this.registry.resolveByName(toolName);

    if (!tool) {
      return this.buildErrorResult(context.invocationId, toolName, 1, startedAt, "TOOL_NOT_FOUND", `Tool '${toolName}' is not registered`);
    }

    // 1. Policy Evaluation
    const policyDecision = this.policyEngine.evaluate(tool, context);
    if (policyDecision.status === "DENY") {
      this.logger.warn(`Policy DENIED tool execution '${toolName}': ${policyDecision.reasons.join("; ")}`);
    ...
    if (context.userId) {
      const rateCheck = await this.rateLimiter.checkUserRateLimit(context.userId);
    ...
    const fingerprint = this.loopGuard.calculateFingerprint(tool.name, validation.data);
    if (tool.cachePolicy.type !== "NONE") {
      const cached = await this.idempotencyService.getCachedResult(fingerprint);
      if (cached) {
        this.logger.log(`Returning cached tool result for '${tool.name}' (fingerprint: ${fingerprint})`);
        return {
          ...cached,
    ...
    if (!executionSuccess) {
      ...
      this.registry.recordHealthTelemetry(tool.name, durationMs, false, errorMessage);
      await this.repository.saveInvocationRecord(tool.name, tool.version, validation.data, errResult, context);
      return errResult;
    }

    // 6. Result Sanitization (Redact Secrets & Enforce Size Bounds)
    if (this.sanitizer.containsSecrets(rawOutput)) {
      this.logger.warn(`Secret pattern detected in output of tool '${tool.name}'. Redacting content.`);
    }
    ...
    if (tool.cachePolicy.type === "SHORT_TTL" || tool.cachePolicy.type === "REQUEST_SCOPE") {
      await this.idempotencyService.setCachedResult(fingerprint, successResult, tool.cachePolicy.ttlSeconds || 10);
    }

    this.registry.recordHealthTelemetry(tool.name, durationMs, true);
    await this.repository.saveInvocationRecord(tool.name, tool.version, validation.data, successResult, context);
  ...
  private buildErrorResult(
    ...
    return {
      ...
      toolName,
      toolVersion,
      status: code === "TOOL_CAPABILITY_DENIED" ? "DENIED" : code === "TOOL_TIMEOUT" ? "TIMED_OUT" : "FAILED",
      error: {
        code,
        message,
        retryable: code === "TOOL_TIMEOUT" || code === "TOOL_DEPENDENCY_UNAVAILABLE",
      },
      metadata: {
        startedAt,
        completedAt,
