...
export class ReflectionRepository {
  ...
  completedRuns(cutoff: Date) {
    return this.prisma.pipelineRun.findMany({
      ...
      select: { id: true, userId: true, symbol: true, provider: true, decision: true, confidence: true, marketRegime: true, configurationVersion: true, learningStage: true, completedAt: true, storedContext: true, result: true, performanceRecords: { select: { horizon: true } } },
