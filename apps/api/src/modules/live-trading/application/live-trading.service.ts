...
import { ExchangeEnvironment, ExchangeProvider, type ExchangePosition, type OrderSide } from "../../../exchange/domain/exchange.types";
...
export class LiveTradingService {
  ...
  async execute(userId: string, dto: ExecuteApprovedOrderDto, context: RequestMetadata) {
    ...
    if (!connection.isEnabled || !connection.isVerified) {
      throw new ForbiddenException("Exchange connection must be enabled and verified");
    }
    ...
    if (!assessment.approved || !assessment.positionSize || !assessment.leverage || assessment.decision === "WAIT") {
      throw new ForbiddenException("Order blocked: risk assessment is not approved for execution");
    }
    if (Date.now() - assessment.createdAt.getTime() > this.config.values.approvalTtlMs) {
    ...
    if (existingApproval) throw new ConflictException("Risk approval has already been consumed");
    const last = await this.prisma.liveOrder.findFirst({
      where: { userId, connectionId: dto.connectionId, status: { not: "FAILED" } },
      orderBy: { createdAt: "desc" },
    });
    if (last && Date.now() - last.createdAt.getTime() < this.config.values.cooldownMs) {
    ...
    const opposite = positions.find((position) => position.side !== desiredSide);
    const configuration = await this.connections.configuration(userId, dto.connectionId, context);
    if (opposite) {
      await this.submit(
        userId,
        connection,
        {
          ...
          reduceOnly: true,
          ...(configuration.positionMode === "HEDGE" ? { positionSide: opposite.side as "LONG" | "SHORT" } : {}),
        },
        "REVERSE",
        null,
        context,
    ...
    const result = await this.submit(
      ...
      connection,
      {
        ...
        ...(configuration.positionMode === "HEDGE" ? { positionSide: desiredSide } : {}),
        ...(assessment.stopLoss ? { stopLoss: String(assessment.stopLoss) } : {}),
        ...(assessment.takeProfit ? { takeProfit: String(assessment.takeProfit) } : {}),
      },
    ...
    await this.sync(userId, dto.connectionId, context).catch((error) =>
      this.logger.warn({ event: "post_order_sync_failed", orderId: result.id, error: this.safeError(error) }),
    );
    return result;
  }
  ...
  async executePipeline(userId: string, pipelineRunId: string) {
    ...
    if (settings.mode !== "DEMO" && settings.mode !== "LIVE") return { outcome: "EXECUTION_MODE_INACTIVE" };
    const assessment = await this.prisma.riskAssessment.findUnique({ where: { pipelineRunId } });
    if (!assessment || assessment.userId !== userId) return { outcome: "RISK_ASSESSMENT_MISSING" };
    if (!assessment.approved) return { outcome: "RISK_REJECTED", reason: assessment.reason };
    ...
    const connection = connections.find((item) =>
      item.isEnabled && item.isVerified &&
      item.environment === (settings.mode === "LIVE" ? ExchangeEnvironment.PRODUCTION : demoEnvironment[item.provider]),
    );
    if (!connection) return { outcome: "NO_ELIGIBLE_EXCHANGE_CONNECTION" };
    const clientOrderId = `p9-${pipelineRunId.replaceAll("-", "").slice(0, 32)}`;
    try {
      const order = await this.execute(userId, { connectionId: connection.id, riskAssessmentId: assessment.id, clientOrderId }, {});
      return { outcome: "ORDER_SUBMITTED", order };
    } catch (error) {
      const safe = this.safeError(error);
      this.logger.error({ event: "pipeline_order_failed", pipelineRunId, errorCode: safe.code });
      return { outcome: "EXECUTION_FAILED", errorCode: safe.code };
    }
  }

  async close(userId: string, dto: CloseApprovedPositionDto, context: RequestMetadata) {
    const connection = await this.connections.get(userId, dto.connectionId);
    this.config.assertExecutionAllowed(connection.environment);
    const assessment = await this.prisma.riskAssessment.findFirst({
      where: { id: dto.riskAssessmentId, userId, symbol: dto.symbol, approved: true },
    });
    if (!assessment || assessment.decision === "WAIT" || !assessment.leverage) {
      throw new ForbiddenException("Position close requires an approved directional risk assessment");
    }
    if (Date.now() - assessment.createdAt.getTime() > this.config.values.approvalTtlMs) {
      throw new ForbiddenException("Order blocked: risk approval has expired");
    }
    ...
    if (assessment.decision === position.side) {
      throw new ForbiddenException("Close approval must point opposite to the current position");
    }
    const configuration = await this.connections.configuration(userId, dto.connectionId, context);
    const result = await this.submit(
      userId,
  ...
  async cancel(userId: string, localOrderId: string, context: RequestMetadata) {
    const row = await this.prisma.liveOrder.findFirst({ where: { id: localOrderId, userId } });
    if (!row) throw new NotFoundException("Order not found");
    if (!row.exchangeOrderId) throw new ConflictException("Order has no exchange identifier");
    const connection = await this.connections.get(userId, row.connectionId);
    const order = await this.connections.cancelOrder(
      userId,
      row.connectionId,
      { symbol: row.symbol, orderId: row.exchangeOrderId, clientOrderId: row.clientOrderId },
      context,
    );
    ...
    await this.audit.record("LIVE_ORDER_CANCELLED", userId, context, { orderId: row.id, exchangeOrderId: row.exchangeOrderId, connectionId: connection.id });
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (positions.length) {
        await tx.livePosition.createMany({
          data: positions.map((position) => this.positionData(userId, connectionId, connection.environment, position, syncedAt)),
        });
      }
      await tx.liveAccountSnapshot.create({
        data: {
          userId, connectionId, provider: connection.provider, environment: connection.environment,
          totalEquity: account.totalEquity, availableBalance: account.availableBalance,
          unrealizedPnl: account.totalUnrealizedPnl, marginBalance: account.totalMarginBalance, syncedAt,
        },
      });
      for (const order of openOrders) {
        await tx.liveOrder.updateMany({
          where: { connectionId, OR: [{ exchangeOrderId: order.exchangeOrderId }, ...(order.clientOrderId ? [{ clientOrderId: order.clientOrderId }] : [])] },
          data: { status: order.status, averagePrice: order.averagePrice },
        });
      }
    });
    await this.monitorProtection(userId, connection, positions, context);
    this.logger.log({ event: "live_state_synced", userId, connectionId, positionCount: positions.length, openOrderCount: openOrders.length });
    return { syncedAt: syncedAt.toISOString(), positions: positions.length, openOrders: openOrders.length };
  }
  ...
  async dashboard(userId: string, connectionId?: string) {
    ...
    const [connections, positions, orders, snapshots] = await Promise.all([
      ...
      this.prisma.livePosition.findMany({ where, orderBy: { updatedAt: "desc" } }),
      this.prisma.liveOrder.findMany({ where, orderBy: { createdAt: "desc" }, take: 100 }),
      this.prisma.liveAccountSnapshot.findMany({ where, orderBy: { syncedAt: "desc" }, take: 100 }),
    ]);
    const latest = [...new Map(snapshots.map((item) => [item.connectionId, item])).values()];
    return {
      ...
      connections,
      accounts: latest.map((row) => ({ ...row, totalEquity: Number(row.totalEquity), availableBalance: Number(row.availableBalance), unrealizedPnl: Number(row.unrealizedPnl), marginBalance: Number(row.marginBalance), syncedAt: row.syncedAt.toISOString() })),
      positions: positions.map((row) => ({ ...row, quantity: Number(row.quantity), entryPrice: Number(row.entryPrice), markPrice: row.markPrice ? Number(row.markPrice) : null, liquidationPrice: row.liquidationPrice ? Number(row.liquidationPrice) : null, unrealizedPnl: Number(row.unrealizedPnl), notional: row.notional ? Number(row.notional) : null, syncedAt: row.syncedAt.toISOString() })),
  ...
  async kill(userId: string, context: RequestMetadata) {
    this.config.kill();
    await this.audit.record("GLOBAL_TRADING_KILL_SWITCH", userId, context, { status: "DISABLED" });
  ...
  private async submit(
    ...
    assessment: { id: string; stopLoss: Prisma.Decimal | null; takeProfit: Prisma.Decimal | null } | null,
    ...
    try {
      row = await this.prisma.liveOrder.create({
        data: {
          userId, connectionId: connection.id, riskAssessmentId: assessment?.id,
          clientOrderId: command.clientOrderId, provider: connection.provider,
          environment: connection.environment, symbol: command.symbol, side: command.side,
          quantity: command.quantity, leverage: command.leverage, status: "SUBMITTING", purpose,
          reduceOnly: command.reduceOnly ?? false, stopLoss: assessment?.stopLoss, takeProfit: assessment?.takeProfit,
        },
      });
    } catch (error) {
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
        throw new ConflictException("Duplicate order or consumed risk approval");
      }
      throw error;
    }
    try {
      const order = await this.connections.placeOrder(userId, connection.id, command, context);
      const updated = await this.prisma.liveOrder.update({
        where: { id: row.id },
        data: { exchangeOrderId: order.exchangeOrderId, averagePrice: order.averagePrice, status: order.status },
      });
      await this.audit.record("LIVE_ORDER_EXECUTED", userId, context, { orderId: updated.id, exchangeOrderId: order.exchangeOrderId, connectionId: connection.id, symbol: command.symbol, side: command.side, purpose });
      this.logger.log({ event: "order_executed", orderId: updated.id, exchangeOrderId: order.exchangeOrderId, symbol: command.symbol, side: command.side, purpose });
      return this.orderView(updated);
    } catch (error) {
      const safe = this.safeError(error);
      await this.prisma.liveOrder.update({ where: { id: row.id }, data: { status: "FAILED", errorCode: safe.code, errorMessage: safe.message } });
      await this.audit.record("LIVE_ORDER_FAILED", userId, context, { orderId: row.id, connectionId: connection.id, symbol: command.symbol, errorCode: safe.code });
      this.logger.error({ event: "order_failed", orderId: row.id, symbol: command.symbol, errorCode: safe.code });
      throw error;
    }
  }

  private positionData(userId: string, connectionId: string, environment: string, position: ExchangePosition, syncedAt: Date) {
    return {
      userId, connectionId, provider: position.provider, environment, symbol: position.symbol, side: position.side,
      quantity: position.quantity, entryPrice: position.entryPrice, markPrice: position.markPrice,
      liquidationPrice: position.liquidationPrice, leverage: position.leverage ? Number(position.leverage) : undefined,
      unrealizedPnl: position.unrealizedPnl, notional: position.notional, syncedAt,
    };
  }
  ...
  private async monitorProtection(
    ...
    for (const position of positions) {
      ...
      if (Number.isFinite(mark) && mark > 0 && Number.isFinite(liquidation) && liquidation > 0) {
        const distance = Math.abs(mark - liquidation) / mark;
        if (distance <= 0.05) {
          this.logger.error({ event: "liquidation_risk", userId, connectionId: connection.id, symbol: position.symbol, distancePct: distance * 100 });
        }
      }
      ...
      if (position.provider !== ExchangeProvider.BINANCE_FUTURES || !Number.isFinite(mark) || mark <= 0) continue;
      ...
      const stopHit = stop !== undefined && (position.side === "LONG" ? mark <= stop : mark >= stop);
      const takeHit = take !== undefined && (position.side === "LONG" ? mark >= take : mark <= take);
      if (!stopHit && !takeHit) continue;
      const purpose = stopHit ? "STOP_LOSS" : "TAKE_PROFIT";
      const clientOrderId = this.derivedId(source.clientOrderId, stopHit ? "sl" : "tp");
  ...
  private orderView(row: { id: string; exchangeOrderId: string | null; clientOrderId: string; provider: string; environment: string; symbol: string; side: string; quantity: Prisma.Decimal; averagePrice: Prisma.Decimal | null; status: string; purpose: string; errorCode: string | null; createdAt: Date; updatedAt: Date }) {
    return { id: row.id, orderId: row.exchangeOrderId, clientOrderId: row.clientOrderId, provider: row.provider, environment: row.environment, symbol: row.symbol, side: row.side, size: Number(row.quantity), price: row.averagePrice ? Number(row.averagePrice) : null, status: row.status, purpose: row.purpose, errorCode: row.errorCode, createdAt: row.createdAt.toISOString(), updatedAt: row.updatedAt.toISOString() };
  }
  ...
  private safeError(error: unknown): { code: string; message: string } {
    return error instanceof ExchangeError
      ...
      : { code: "EXECUTION_FAILED", message: "Exchange order execution failed" };
  }
}
