...
export class MetadataExtractor {
  ...
  private readonly ambiguousSymbols = new Set(['ONE', 'LINK', 'NEAR', 'GAS', 'OP', 'FOR', 'KEY', 'TIME', 'MANA', 'RUN']);

  private readonly topicKeywords: Record<string, string[]> = {
    regulation: ['sec', 'cftc', 'regulator', 'regulation', 'legal', 'lawsuit', 'subpoena', 'enforcement', 'compliance', 'court'],
    ETF: ['etf', 'spot etf', 'blackrock', 'fidelity', 'grayscale', 's-1', '19b-4'],
    listing: ['list', 'listing', 'lists', 'new pair', 'trading pair', 'launchpool'],
    delisting: ['delist', 'delisting', 'delists', 'cease trading', 'removal'],
    security: ['exploit', 'hack', 'hacked', 'vulnerability', 'attack', 'compromised', 'stolen', 'phishing', 'security notice'],
    stablecoin: ['usdt', 'usdc', 'dai', 'tether', 'circle', 'depeg', 'stablecoin'],
    macro: ['cpi', 'ppi', 'fomc', 'fed', 'federal reserve', 'interest rate', 'inflation', 'gdp', 'unemployment', 'payroll'],
    institutional_adoption: ['institutional', 'treasury', 'microstrategy', 'corporate reserve', 'hedge fund'],
    DeFi: ['defi', 'decentralized finance', 'tvl', 'yield', 'liquidity pool', 'dex', 'uniswap', 'aave'],
    layer_1: ['layer 1', 'l1', 'bitcoin', 'ethereum', 'solana', 'avalanche'],
    layer_2: ['layer 2', 'l2', 'arbitrum', 'optimism', 'base', 'zksync', 'starknet', 'rollup'],
    derivatives: ['futures', 'perpetual', 'funding rate', 'open interest', 'options', 'margin'],
  ...
  private readonly entityKeywords: { name: string; type: string; keywords: string[] }[] = [
    { name: 'Binance', type: 'Exchange', keywords: ['binance', 'cz', 'richard teng'] },
    { name: 'OKX', type: 'Exchange', keywords: ['okx', 'okex'] },
    { name: 'Coinbase', type: 'Exchange', keywords: ['coinbase', 'brian armstrong'] },
    { name: 'Bybit', type: 'Exchange', keywords: ['bybit'] },
    { name: 'Kraken', type: 'Exchange', keywords: ['kraken'] },
    { name: 'SEC', type: 'Regulator', keywords: ['sec', 'securities and exchange commission', 'gary gensler'] },
    { name: 'CFTC', type: 'Regulator', keywords: ['cftc', 'commodity futures trading commission'] },
    { name: 'Fed', type: 'CentralBank', keywords: ['fed', 'federal reserve', 'powell', 'jerome powell'] },
  ...
  extractMetadata(title: string, summary: string = ''): ExtractedMetadata {
    ...
    for (const [ticker, mappedPair] of Object.entries(this.symbolMap)) {
      if (this.ambiguousSymbols.has(ticker)) {
        ...
        const pairRegex = new RegExp(`\\b${ticker}[/-](USDT|USD|BUSD|PERP)\\b`, 'i');
    ...
    const entities = Array.from(entitiesMap.entries()).map(([entity, entityType]) => ({
      entity,
      entityType,
    }));
