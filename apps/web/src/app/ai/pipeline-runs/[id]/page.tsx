    ...
    <div className="space-y-6">
      <div className="flex justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-bold">{run.symbol} pipeline</h1>
        ...
        <div className="flex gap-2">
          <button
            ...
            onClick={() =>
              void action(`/pipeline-runs/${id}/replay`, {
                mode: "REPLAY_WITH_STORED_CONTEXT",
              })
            }
          >
            Replay stored
          ...
          {["QUEUED", "RUNNING"].includes(run.status) && (
            <button
              ...
              onClick={() => void action(`/pipeline-runs/${id}/cancel`)}
            >
              Cancel
            </button>
          )}
      ...
      {!!run.alerts.length && (
        <section className="rounded-lg border bg-card p-5">
          ...
          {run.alerts.map((alert) => (
            <div
              className="mt-3 rounded border border-amber-500/30 p-3 text-sm"
              key={alert.id}
            >
              <strong>{alert.kind}</strong>
              <p>{alert.reasoningSummary}</p>
            </div>
          ))}
        </section>
      )}
